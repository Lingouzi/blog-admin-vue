...
const chartsConfig = window.typeConfig
...
function initEvent () {
    let cacheValue = null
    // 图表类型数
    const typeViewCount = chartsConfig.length - 1
    const $chartsTypeViewBox = $('#scrollBed .view-box')
    ...
    $('#scrollBed').delegate('.view-box', 'click', function (e) {
        const index = $(this).attr('data-chart-type')
...
function getTableDataFormat () {
    const form = document.forms[ 'data-form' ]
    const items = form['charts-format']
...
function getSeriesForPieChart () {
    ...
    for (let j = 1, jlen = tableData[ 0 ].length; j < jlen; j++) {
        const title = tableData[ 0 ][ j ]
        const val = tableData[ 1 ][ j ]
...
function getTableData () {
    ...
    const xCount = table.rows[0].cells.length - 1
    const values = getTableInputValue()
...
function getCellValue (cell) {
    const value = utils.trim((cell.innerText || cell.textContent || ''))

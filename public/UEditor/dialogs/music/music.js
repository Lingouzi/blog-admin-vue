...
(function () {
    ...
    Music.prototype = {
        ...
        listenTest: function (elem) {
            const me = this
            const view = $G('J_preview')
            const is_play_action = (elem.className == 'm-try')
            const old_trying = me._getTryingElem()
        ...
        _getUrl: function (isTryListen) {
            const me = this
            const param = 'from=tiebasongwidget&url=&name=' + encodeURIComponent(me._removeHtml(selectedItem.title)) + '&artist=' +
        ...
        _getTryingElem: function () {
            const s = $G('J_listPanel').getElementsByTagName('span')

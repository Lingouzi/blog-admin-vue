 ...
 */
const scrawl = function (options) {
    options && this.initOptions(options)
};
(function () {
    const canvas = $G('J_brushBoard')
    const context = canvas.getContext('2d')
    let drawStep = [] // undo redo存储
    let drawStepIndex = 0 // undo redo指针
    ...
    scrawl.prototype = {
        ...
        _addBoardListener: function (saveNum) {
            ...
            let isMouseUp = false
            let buttonPress = 0; let button; let flag = ''
        ...
        _addColorBarListener: function () {
            ...
            domUtils.on($G('J_colorBar'), 'click', function (e) {
                const target = me.getTarget(e)
                const color = target.title
                if (color) {
        ...
        _addBrushBarListener: function () {
            ...
            domUtils.on($G('J_brushBar'), 'click', function (e) {
                const target = me.getTarget(e)
                const size = browser.ie ? target.innerText : target.text
                if (size) {
        ...
        _addScalePicListenter: function () {
            domUtils.on($G('J_sacleBoard'), 'click', function () {
                const picBoard = $G('J_picBoard')
                const scaleCon = $G('J_scaleCon')
                const img = picBoard.children[0]
        ...
        _clearSelection: function () {
            const list = ['J_operateBar', 'J_colorBar', 'J_brushBar', 'J_eraserBar', 'J_picBoard']
        ...
        _originalColorSelect: function (title) {
            const colorList = $G('J_colorList').getElementsByTagName('td')
        ...
        _originalBrushSelect: function (text) {
            const brushList = $G('J_brushBar').children
            for (var i = 0, ele; ele = brushList[i++];) {
                if (ele.tagName.toLowerCase() == 'a') {
                    const size = browser.ie ? ele.innerText : ele.text
        ...
        _addColorSelect: function (target) {
            const me = this
            const colorList = $G('J_colorList').getElementsByTagName('td')
            const eraserList = $G('J_eraserBar').children
            const brushList = $G('J_brushBar').children
        ...
        getCanvasData: function () {
            const picContainer = $G('J_picBoard')
...
(function () {
    function _appendStyle() {
        ...
        const cssText = '.scale{visibility:hidden;cursor:move;position:absolute;left:0;top:0;width:100px;height:50px;background-color:#fff;font-size:0;line-height:0;opacity:.4;filter:Alpha(opacity=40);}' +
    ...
    ScaleBoy.prototype = {
        ...
        startScale: function (objElement) {
            const me = this; const Idom = me.dom
        ...
        updateScaledElement: function (objStyle) {
            const cur = this.scalingElement
            const pos = objStyle.position
            const size = objStyle.size
            if (pos) {
        ...
        scaleMousemove: function (e) {
            const me = arguments.callee.fp
            const start = me.start
            const dir = me.dir || 'def'
...
function ue_callback(url, state) {
    const doc = document
    const picBorard = $G('J_picBoard')
    const img = doc.createElement('img')

    // 图片缩放
    function scale(img, max, oWidth, oHeight) {
        let width = 0; let height = 0; let percent; const ow = img.width || oWidth; const oh = img.height || oHeight
        if (ow > max || oh > max) {
            if (ow >= oh) {
                if (width = ow - max) {
...
function exec(scrawlObj) {
    if (scrawlObj.isScrawl) {
        ...
        const base64 = scrawlObj.getCanvasData()
        if (base64) {
            const options = {
                timeout: 100000,
                onsuccess: function (xhr) {
                    if (!scrawlObj.isCancelScrawl) {
                        ...
                        if (responseObj.state == 'SUCCESS') {
                            const imgObj = {}
                            const url = editor.options.scrawlUrlPrefix + responseObj.url
                            imgObj.src = url
                            imgObj._src = url
            ...
            const actionUrl = editor.getActionUrl(editor.getOpt('scrawlActionName'))
            const params = utils.serializeParam(editor.queryCommandValue('serverparam')) || ''
            const url = utils.formatUrl(actionUrl + (actionUrl.indexOf('?') == -1 ? '?' : '&') + params)

 ...
 */
(function () {
    const title = $G('J_title')
    const titleCol = $G('J_titleCol')
    const caption = $G('J_caption')
    const sorttable = $G('J_sorttable')
    const autoSizeContent = $G('J_autoSizeContent')
    const autoSizePage = $G('J_autoSizePage')
    const tone = $G('J_tone')
    let me
    const preview = $G('J_preview')
    ...
    editTable.prototype = {
        init: function () {
            const colorPiker = new UE.ui.ColorPicker({
                editor: editor
            })
            const colorPop = new UE.ui.Popup({
                editor: editor,
                content: colorPiker
            })
            ...
            const enablesortState = editor.queryCommandState('enablesort')
            const disablesortState = editor.queryCommandState('disablesort')
        ...
        titleHanler: function () {
            const example = $G('J_example')
            const frg = document.createDocumentFragment()
            const color = domUtils.getComputedStyle(domUtils.getElementsByTagName(example, 'td')[0], 'border-color')
            const colCount = example.rows[0].children.length
        ...
        titleColHanler: function () {
            const example = $G('J_example')
            const color = domUtils.getComputedStyle(domUtils.getElementsByTagName(example, 'td')[0], 'border-color')
            const colArr = example.rows
            const colCount = colArr.length
        ...
        captionHanler: function () {
            const example = $G('J_example')
            if (caption.checked) {
                const row = document.createElement('caption')
        ...
        autoSizePageHanler: function () {
            const example = $G('J_example')
            const tds = example.getElementsByTagName(example, 'td')
        ...
        updateSortSpan: function() {
            ...
            const spans = domUtils.getElementsByTagName(example, 'span')
        ...
        getColor: function () {
            const start = editor.selection.getStart(); let color
            const cell = domUtils.findParentByTagName(start, ['td', 'th', 'caption'], true)
        ...
        setColor: function (color) {
            const example = $G('J_example')
            const arr = domUtils.getElementsByTagName(example, 'td').concat(

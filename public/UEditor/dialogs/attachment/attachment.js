...
(function () {
    ...
    function setTabFocus(id) {
        if (!id) return
        let i; let bodyId; const tabs = $G('tabhead').children
    ...
    function initButtons() {
        dialog.onok = function () {
            let list = []; let id; const tabs = $G('tabhead').children
            for (let i = 0; i < tabs.length; i++) {
    ...
    UploadFile.prototype = {
        ...
        initUploader: function () {
            ...
            // WebUploader实例
            let uploader
            const actionUrl = editor.getActionUrl(editor.getOpt('fileActionName'))
            const fileMaxSize = editor.getOpt('fileMaxSize')
            const acceptExtensions = (editor.getOpt('fileAllowFiles') || []).join('').replace(/\./g, ',')
                .replace(/^[,]/, '')
        ...
        getInsertList: function () {
            let i; let link; let data; const list = []
            const prefix = editor.getOpt('fileUrlPrefix')
    ...
    OnlineFile.prototype = {
            ...
            if (!_this.listEnd && !this.isLoadingData) {
                ...
                ajax.request(editor.getActionUrl(editor.getOpt('fileManagerActionName')), {
                    ...
                    onsuccess: function (r) {
                        try {
                            const json = eval('(' + r.responseText + ')')
                            if (json.state == 'SUCCESS') {
        ...
        pushData: function (list) {
            let i; let item; let img; let filetype; let preview; let icon; const _this = this
            const urlPrefix = editor.getOpt('fileManagerUrlPrefix')
        ...
        getInsertList: function () {
            let i; const lis = this.list.children; const list = []
            for (i = 0; i < lis.length; i++) {
                if (domUtils.hasClass(lis[i], 'selected')) {
                    const url = lis[i].getAttribute('data-url')
                    const title = lis[i].getAttribute('data-title') || url.substr(url.lastIndexOf('/') + 1)
                    list.push({
                        title: title,
                        url: url
                    })

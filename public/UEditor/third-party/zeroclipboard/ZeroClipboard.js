...
(function(window) {
    ...
    const _swfPath = (function() {
        let i; let jsDir; let tmpJsPath; let jsPath; let swfPath = 'ZeroClipboard.swf'
    ...
    const _camelizeCssPropName = (function() {
        const matcherRegex = /\-([a-z])/g; const replacerFn = function(match, group) {
        ...
        if (value && typeof value === 'string') {
            ...
            if (element.nodeType === 1) {
                if (!element.className) {
                    ...
                    const className = ' ' + element.className + ' '; let setClass = element.className
    ...
    const _getZoomFactor = function() {
        let rect; let physicalWidth; let logicalWidth; let zoomFactor = 1
    ...
    const _getDOMObjectPosition = function(obj, defaultZIndex) {
        ...
        if (obj.getBoundingClientRect) {
            ...
            const leftBorderWidth = document.documentElement.clientLeft || 0
            const topBorderWidth = document.documentElement.clientTop || 0
    ...
    const _vars = function(options) {
        let i; let len; let domain; let domains; let str = ''; let trustedOriginsExpanded = []
    ...
    const _extend = function() {
        let i; let len; let arg; let prop; let src; let copy; const target = arguments[0] || {}
    ...
    const _determineScriptAccess = (function() {
        const _extractAllDomains = function(origins, resultsArray) {
            let i, len, tmp
    ...
    const _mapClipDataToFlash = function(clipData) {
        const newClipData = {}; const formatMap = {}
    ...
    const _detectFlashSupport = function() {
        let plugin; let ax; let mimeType; let hasFlash = false; let isActiveX = false; let isPPAPI = false; let flashVersion = ''
    ...
    const _getAllClients = function() {
        let i; let len; let client; const clients = []; const clientIds = _objectKeys(_clientMeta)
    ...
    ZeroClipboard.config = function(options) {
        ...
        const copy = {}
        for (const prop in _globalConfig) {
    ...
    ZeroClipboard.destroy = function() {
        ...
        for (const clientId in _clientMeta) {
    ...
    var _bridge = function() {
        ...
        if (!container) {
            const allowScriptAccess = _determineScriptAccess(window.location.host, _globalConfig)
            const allowNetworking = allowScriptAccess === 'never' ? 'none' : 'all'
            const flashvars = _vars(_globalConfig)
            const swfUrl = _globalConfig.swfPath + _cacheBust(_globalConfig.swfPath, _globalConfig)
            container = _createHtmlBridge()
            const divToBeReplaced = document.createElement('div')
            container.appendChild(divToBeReplaced)
            document.body.appendChild(container)
            let tmpDiv = document.createElement('div')
            const oldIE = _flashState.pluginType === 'activex'
    ...
    var _reposition = function() {
        if (_currentElement) {
            const pos = _getDOMObjectPosition(_currentElement, _globalConfig.zIndex)
            const htmlBridge = _getHtmlBridge(_flashState.bridge)
            if (htmlBridge) {
    ...
    var _dispatchClientCallbacks = function(event, async) {
        const handlers = _clientMeta[this.id] && _clientMeta[this.id].handlers[event.type]
        if (handlers && handlers.length) {
            let i; let len; let func; let context; const originalContext = this
    ...
    var _preprocessEvent = function(event) {
        const element = event.target || _currentElement
        switch (event.type) {
            case 'error':
    ...
    ZeroClipboard.prototype.on = function(eventName, func) {
        let i; let len; let events; const added = {}; const handlers = _clientMeta[this.id] && _clientMeta[this.id].handlers
    ...
    ZeroClipboard.prototype.off = function(eventName, func) {
        let i; let len; let foundIndex; let events; let perEventHandlers; const handlers = _clientMeta[this.id] && _clientMeta[this.id].handlers
    ...
    ZeroClipboard.prototype.handlers = function(eventName) {
        let prop; let copy = null; const handlers = _clientMeta[this.id] && _clientMeta[this.id].handlers
        if (handlers) {
            if (typeof eventName === 'string' && eventName) {
    ...
    var _getAllClientsClippedToElement = function(element) {
        let elementMetaId; let clientIds; let i; let len; let client; const clients = []

...
(function (undefined) {
    ...
    XRegExp = function (pattern, flags) {
        ...
        let pos = 0
        let context; let tokenResult; let match; let chr; let regex
    ...
    String.prototype.replace = function (search, replacement) {
        ...
        let captureNames; let result; let str; let origLastIndex
...
if (typeof (SyntaxHighlighter) === 'undefined') {
    var SyntaxHighlighter = (function() {
        ...
        var sh = {
             ...
             */
                getHtml: function(highlighter) {
                    ...
                    for (let i = 0; i < list.length; i++) { html += (items[list[i]].getHtml || defaultGetHtml)(highlighter, list[i]) }
             ...
             */
                handler: function(e) {
                    ...
                    function getValue(name) {
                        const r = new RegExp(name + '_(\\w+)')
                        const match = r.exec(className)

                        return match ? match[1] : null
                    }

                    const highlighter = getHighlighterById(findParentElement(target, '.syntaxhighlighter').id)
                    const commandName = getValue('command')
                ...
                // Ordered lis of items in the toolbar. Can't expect `for (var n in items)` to be consistent.
                    ...
                    help: {
                        execute: function(highlighter) {
                            const wnd = popup('', '_blank', 500, 250, 'scrollbars=0')
         ...
         */
            highlight: function(globalParams, element) {
                const elements = this.findElements(globalParams, element)
                ...
                for (let i = 0; i < elements.length; i++) {
                    ...
                    const tmp = element.firstChild.firstChild
     ...
     */
        function findElement(target, search, reverse /* optional */) {
            ...
            for (let i = 0; nodes && i < nodes.length && found == null; i++) { found = findElement(nodes[i], search, reverse) }
     ...
     */
        function findBrush(alias, showAlert) {
            ...
            if (brushes == null) {
                ...
                // Find all brushes
                for (const brush in sh.brushes) {
                    const info = sh.brushes[brush]
                    ...
                    for (let i = 0; i < aliases.length; i++) { brushes[aliases[i]] = brush }
                }
     ...
     */
        function eachLine(str, callback) {
            ...
            for (let i = 0; i < lines.length; i++) { lines[i] = callback(lines[i], i) }
     ...
     */
        function parseParams(str) {
            let match
            const result = {}
            const arrayRegex = new XRegExp('^\\[(?<values>(.*?))\\]$')
     ...
     */
        function wrapLinesWithCode(str, css) {
            ...
            str = str.replace(/ {2,}/g, function(m) {
                ...
                for (let i = 0; i < m.length - 1; i++) { spaces += sh.config.space }
     ...
     */
        function processSmartTabs(code, tabSize) {
            ...
            for (let i = 0; i < 50; i++) { spaces += '                    ' } // 20 spaces * 50
            ...
            code = eachLine(code, function(line) {
                ...
                // calculate the amount of spaces we need to insert.
                    const spaces = tabSize - pos % tabSize
     ...
     */
        function unindent(str) {
            const lines = splitLines(fixInputString(str))
     ...
     */
        function getMatches(code, regexInfo) {
            ...
            let match = null
            let matches = []
            const func = regexInfo.func ? regexInfo.func : defaultAdd
     ...
     */
        function quickCodeHandler(e) {
            ...
            for (let i = 0; i < lines.length; i++) { code.push(lines[i].innerText || lines[i].textContent) }
     ...
     */
        sh.HtmlScript = function(scriptBrushName) {
            const brushClass = findBrush(scriptBrushName)
            let scriptBrush
            const xmlBrush = new sh.brushes.Xml()
            const bracketsRegex = null
            const ref = this
            const methodsToExpose = 'getDiv getHtml init'.split(' ')
            ...
            function offsetMatches(matches, offset) {
                for (let j = 0; j < matches.length; j++) { matches[j].index += offset }
            }
            ...
            function process(match, info) {
                ...
                for (let j = 0; j < matches.length; j++) { matches[j].brushName = brushClass.brushName }
     ...
     */
         ...
         */
            ...
            // Optimized by Jose Prado (http://joseprado.com)
                for (let i = 0; i < matches.length; i++) {
                    ...
                    const itemIEndPos = itemI.index + itemI.length
         ...
         */
            figureOutLineNumbers: function(code) {
                const lines = []
                const firstLine = parseInt(this.getParam('first-line'))
         ...
         */
            getLineNumbersHtml: function(code, lineNumbers) {
                ...
                const firstLine = parseInt(this.getParam('first-line'))
                let pad = this.getParam('pad-line-numbers')

                if (pad == true) { pad = (firstLine + count - 1).toString().length } else if (isNaN(pad) == true) { pad = 0 }

                for (let i = 0; i < count; i++) {
                    const lineNumber = lineNumbers ? lineNumbers[i] : firstLine + i
         ...
         */
            getCodeLinesHtml: function(html, lineNumbers) {
                ...
                const padLength = this.getParam('pad-line-numbers')
                const firstLine = parseInt(this.getParam('first-line'))
                var html = ''
                const brushName = this.getParam('brush')
         ...
         */
            getMatchesHtml: function(code, matches) {
                ...
                function getBrushNameCss(match) {
                    const result = match ? (match.brushName || brushName) : brushName
...
;(function() {
    ...
    function Brush() {
        ...
        const keywords =	'auto break case catch class const decltype __finally __exception __try ' +
...
;(function() {
    ...
    function Brush() {
        function process(match, regexInfo) {
            ...
            const code = match[0]
            const tag = new XRegExp('(&lt;|<)[\\s\\/\\?]*(?<name>[:\\w-\\.]+)', 'xg').exec(code)
            const result = []

            if (match.attributes != null) {
                let attributes
                const regex = new XRegExp('(?<name> [\\w:\\-\\.]+)' +
										'\\s*=\\s*' +
										'(?<value> ".*?"|\'.*?\'|\\w+)',
                'xg')

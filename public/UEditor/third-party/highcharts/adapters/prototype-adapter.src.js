...
var HighchartsAdapter = (function () {
    const hasEffect = typeof Effect !== 'undefined'
	 ...
	 */
        getScript: function (scriptLocation, callback) {
            const head = $$('head')[0] // Returns an array, so pick the first element.
            if (head) {
	 ...
	 */
        addNS: function (eventName) {
            const HTMLEvents = /^(?:load|unload|abort|error|select|change|submit|reset|focus|blur|resize|scroll)$/
            const MouseEvents = /^(?:click|mouse(?:down|up|over|move|out))$/
            ...
            if (!object._highcharts_extended) {
                Object.extend(object, {
                    ...
                    _highcharts_stop_observing: function (name, fn) {
                        if (name) {
                            if (fn) {
                                this._highchart_events[name] = [this._highchart_events[name]].compact().flatten()
                                    .without(fn)
                            } else {

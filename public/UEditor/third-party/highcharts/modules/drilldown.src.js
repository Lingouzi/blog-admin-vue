...
(function (H) {
    ...
    const ColumnSeries = seriesTypes.column
    const fireEvent = HighchartsAdapter.fireEvent
    ...
    Chart.prototype.addSeriesAsDrilldown = function (point, ddOptions) {
        const oldSeries = point.series
        const xAxis = oldSeries.xAxis
        const yAxis = oldSeries.yAxis
        let newSeries
        const color = point.color || oldSeries.color
        let pointIndex
        let level
    ...
    Chart.prototype.getDrilldownBackText = function () {
        const lastLevel = this.drilldownLevels[this.drilldownLevels.length - 1]
    ...
    Chart.prototype.showDrillUpButton = function () {
        const chart = this
        const backText = this.getDrilldownBackText()
        const buttonOptions = chart.options.drilldown.drillUpButton
    ...
    Chart.prototype.drillUp = function () {
        const chart = this
        const level = chart.drilldownLevels.pop()
        const oldSeries = chart.series[0]
        const oldExtremes = level.oldExtremes
        const newSeries = chart.addSeries(level.seriesOptions, false)
    ...
    PieSeries.prototype.animateDrilldown = function (init) {
        ...
        const start = animateFrom.start
        const angle = animateFrom.end - start
        const startAngle = angle / this.points.length
        const startColor = H.Color(level.color).rgba

        if (!init) {
            each(this.points, function (point, i) {
                const endColor = H.Color(point.color).rgba
    ...
    PieSeries.prototype.animateDrillupTo =
			ColumnSeries.prototype.animateDrillupTo = function (init) {
			    if (!init) {
			        ...
			        const level = newSeries.drilldownLevel
			        ...
			        setTimeout(function () {
			            each(newSeries.points, function (point, i) {
			                ...
			                const verb = i === level.pointIndex ? 'show' : 'fadeIn'
			                point.graphic[verb]()
			                if (point.dataLabel) {
    ...
    ColumnSeries.prototype.animateDrilldown = function (init) {
        const animateFrom = this.chart.drilldownLevels[this.chart.drilldownLevels.length - 1].shapeArgs
        const animationOptions = this.chart.options.drilldown.animation
	...
	function (level) {
	    ...
	    each(this.points, function (point) {
	        ...
	        const startColor = H.Color(point.color).rgba
    ...
    H.Point.prototype.doDrilldown = function () {
        ...
        const drilldown = chart.options.drilldown
    ...
    wrap(H.Point.prototype, 'init', function (proceed, series, options, x) {
        const point = proceed.call(this, series, options, x)
        const chart = series.chart
        const tick = series.xAxis && series.xAxis.ticks[x]
        const tickLabel = tick && tick.label
    ...
    let type
    const drawTrackerWrapper = function (proceed) {
        proceed.call(this)
        each(this.points, function (point) {

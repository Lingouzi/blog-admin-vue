...
*/
function RGBColor(m) {
    this.ok = !1; m.charAt(0) == '#' && (m = m.substr(1, 6)); var m = m.replace(/ /g, ''); var m = m.toLowerCase(); const a = { aliceblue: 'f0f8ff', antiquewhite: 'faebd7', aqua: '00ffff', aquamarine: '7fffd4', azure: 'f0ffff', beige: 'f5f5dc', bisque: 'ffe4c4', black: '000000', blanchedalmond: 'ffebcd', blue: '0000ff', blueviolet: '8a2be2', brown: 'a52a2a', burlywood: 'deb887', cadetblue: '5f9ea0', chartreuse: '7fff00', chocolate: 'd2691e', coral: 'ff7f50', cornflowerblue: '6495ed', cornsilk: 'fff8dc', crimson: 'dc143c', cyan: '00ffff', darkblue: '00008b',
        ...
        slategray: '708090', snow: 'fffafa', springgreen: '00ff7f', steelblue: '4682b4', tan: 'd2b48c', teal: '008080', thistle: 'd8bfd8', tomato: 'ff6347', turquoise: '40e0d0', violet: 'ee82ee', violetred: 'd02090', wheat: 'f5deb3', white: 'ffffff', whitesmoke: 'f5f5f5', yellow: 'ffff00', yellowgreen: '9acd32' }; let c; for (c in a)m == c && (m = a[c]); const d = [{ re: /^rgb\((\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3})\)$/, example: ['rgb(123, 234, 45)', 'rgb(255,234,245)'], process: function(b) { return [parseInt(b[1]), parseInt(b[2]), parseInt(b[3])] } }, { re: /^(\w{2})(\w{2})(\w{2})$/,
        example: ['#00ff00', '336699'], process: function(b) { return [parseInt(b[1], 16), parseInt(b[2], 16), parseInt(b[3], 16)] } }, { re: /^(\w{1})(\w{1})(\w{1})$/, example: ['#fb0', 'f0f'], process: function(b) { return [parseInt(b[1] + b[1], 16), parseInt(b[2] + b[2], 16), parseInt(b[3] + b[3], 16)] } }]; for (c = 0; c < d.length; c++) { const b = d[c].process; const k = d[c].re.exec(m); if (k)channels = b(k), this.r = channels[0], this.g = channels[1], this.b = channels[2], this.ok = !0 } this.r = this.r < 0 || isNaN(this.r) ? 0 : this.r > 255 ? 255 : this.r; this.g = this.g < 0 || isNaN(this.g) ? 0
        : this.g > 255 ? 255 : this.g; this.b = this.b < 0 || isNaN(this.b) ? 0 : this.b > 255 ? 255 : this.b; this.toRGB = function() { return 'rgb(' + this.r + ', ' + this.g + ', ' + this.b + ')' }; this.toHex = function() { let b = this.r.toString(16); let a = this.g.toString(16); let d = this.b.toString(16); b.length == 1 && (b = '0' + b); a.length == 1 && (a = '0' + a); d.length == 1 && (d = '0' + d); return '#' + b + a + d }; this.getHelpXML = function() {
        for (var b = [], k = 0; k < d.length; k++) for (var c = d[k].example, j = 0; j < c.length; j++)b[b.length] = c[j]; for (const h in a)b[b.length] = h; c = document.createElement('ul')
        c.setAttribute('id', 'rgbcolor-examples'); for (k = 0; k < b.length; k++) try { const l = document.createElement('li'); const o = new RGBColor(b[k]); const n = document.createElement('div'); n.style.cssText = 'margin: 3px; border: 1px solid black; background:' + o.toHex() + '; color:' + o.toHex(); n.appendChild(document.createTextNode('test')); const q = document.createTextNode(' ' + b[k] + ' -> ' + o.toRGB() + ' -> ' + o.toHex()); l.appendChild(n); l.appendChild(q); c.appendChild(l) } catch (p) {} return c
    }
}
if (!window.console)window.console = {}, window.console.log = function() {}, window.console.dir = function() {}; if (!Array.prototype.indexOf)Array.prototype.indexOf = function(m) { for (let a = 0; a < this.length; a++) if (this[a] == m) return a; return -1 };
(function() {
    function m() {
        const a = { FRAMERATE: 30, MAX_VIRTUAL_PIXELS: 3E4 }; a.init = function(c) {
        ...
        }; a.init(); a.ImagesLoaded = function() { for (let c = 0; c < a.Images.length; c++) if (!a.Images[c].loaded) return !1; return !0 }; a.trim = function(a) { return a.replace(/^\s+|\s+$/g, '') }; a.compressSpaces = function(a) { return a.replace(/[\s\r\t\n]+/gm, ' ') }; a.ajax = function(a) {
        ...
        }; a.parseXml = function(a) { if (window.DOMParser) return (new DOMParser()).parseFromString(a, 'text/xml'); else { var a = a.replace(/<!DOCTYPE svg[^>]*>/, ''); const d = new ActiveXObject('Microsoft.XMLDOM'); d.async = 'false'; d.loadXML(a); return d } }; a.Property = function(c, d) {
...
(b /= 100); return b
            }; this.valueOrDefault = function(b) { return this.hasValue() ? this.value : b }; this.numValueOrDefault = function(b) { return this.hasValue() ? this.numValue() : b }; const b = this; this.Color = { addOpacity: function(d) { let c = b.value; if (d != null && d != '') { const f = new RGBColor(b.value); f.ok && (c = 'rgba(' + f.r + ', ' + f.g + ', ' + f.b + ', ' + d + ')') } return new a.Property(b.name, c) } }; this.Definition = { getDefinition: function() { const d = b.value.replace(/^(url\()?#([^\)]+)\)?$/, '$2'); return a.Definitions[d] }, isUrl: function() {
                return b.value.indexOf('url(') ==
0
            }, getFillStyle: function(b) { const d = this.getDefinition(); return d != null && d.createGradient ? d.createGradient(a.ctx, b) : d != null && d.createPattern ? d.createPattern(a.ctx, b) : null } }; this.Length = { DPI: function() { return 96 }, EM: function(b) { let d = 12; const c = new a.Property('fontSize', a.Font.Parse(a.ctx.font).fontSize); c.hasValue() && (d = c.Length.toPixels(b)); return d }, toPixels: function(d) {
                if (!b.hasValue()) return 0; const c = b.value + ''; return c.match(/em$/) ? b.numValue() * this.EM(d) : c.match(/ex$/) ? b.numValue() * this.EM(d) / 2 : c.match(/px$/)
            ...
            } }; this.Time = { toMilliseconds: function() { if (!b.hasValue()) return 0; const a = b.value + ''; if (a.match(/s$/)) return b.numValue() * 1E3; a.match(/ms$/); return b.numValue() } }; this.Angle = { toRadians: function() {
                if (!b.hasValue()) return 0; const a = b.value +
...
g.fontFamily, fontSize: e || g.fontSize, fontStyle: d || g.fontStyle, fontWeight: c || g.fontWeight, fontVariant: b || g.fontVariant, toString: function() { return [this.fontStyle, this.fontVariant, this.fontWeight, this.fontSize, this.fontFamily].join(' ') } }
            }; const c = this; this.Parse = function(d) {
...
function(b) { const a = this.x * b[1] + this.y * b[3] + b[5]; this.x = this.x * b[0] + this.y * b[2] + b[4]; this.y = a }
...
a); this.addBezierCurve(b, a, d, d + 1 / 3 * (k - b), c, c + 1 / 3 * (l - a), k, l)
                ...
                const q = [b, a]; const p = [d, c]; const t = [k, l]; const m = [o, n]; this.addPoint(q[0], q[1]); this.addPoint(m[0], m[1]); for (i = 0; i <= 1; i++) {
...
(-a + Math.sqrt(c)) / (2 * d), k > 0 && k < 1 && (i == 0 && this.addX(b(k)), i == 1 && this.addY(b(k))), a = (-a - Math.sqrt(c)) / (2 * d), a > 0 && a < 1 && (i == 0 && this.addX(b(a)), i == 1 && this.addY(b(a)))))
            ...
            const d = this; this.Type = {}; this.Type.translate = function(b) {
                ...
                b = a.ToNumberArray(b); this.angle = new a.Property('angle', b[0]); this.cx = b[1] || 0; this.cy = b[2] || 0; this.apply = function(b) { b.translate(this.cx, this.cy); b.rotate(this.angle.Angle.toRadians()); b.translate(-this.cx, -this.cy) }; this.applyToPoint = function(b) {
                    const a = this.angle.Angle.toRadians(); b.applyTransform([1, 0, 0, 1, this.p.x || 0, this.p.y || 0]); b.applyTransform([Math.cos(a), Math.sin(a), -Math.sin(a), Math.cos(a), 0, 0]); b.applyTransform([1, 0, 0, 1, -this.p.x ||
0, -this.p.y || 0])
                }
...
new a.Property('angle', b)
            }; this.Type.SkewBase.prototype = new this.Type.matrix(); this.Type.skewX = function(b) { this.base = d.Type.SkewBase; this.base(b); this.m = [1, 0, Math.tan(this.angle.Angle.toRadians()), 1, 0, 0] }; this.Type.skewX.prototype = new this.Type.SkewBase(); this.Type.skewY = function(b) { this.base = d.Type.SkewBase; this.base(b); this.m = [1, Math.tan(this.angle.Angle.toRadians()), 0, 1, 0, 0] }; this.Type.skewY.prototype = new this.Type.SkewBase(); this.transforms = []; this.apply = function(b) { for (let a = 0; a < this.transforms.length; a++) this.transforms[a].apply(b) }
            this.applyToPoint = function(b) { for (let a = 0; a < this.transforms.length; a++) this.transforms[a].applyToPoint(b) }; for (var c = a.trim(a.compressSpaces(c)).split(/\s(?=[a-z])/), b = 0; b < c.length; b++) { const k = c[b].split('(')[0]; const e = c[b].split('(')[1].replace(')', ''); this.transforms.push(new this.Type[k](e)) }
        }; a.AspectRatio = function(c, d, b, k, e, f, g, j, h, l) {
            var d = a.compressSpaces(d); var d = d.replace(/^defer\s/, ''); const o = d.split(' ')[0] || 'xMidYMid'; var d = d.split(' ')[1] || 'meet'; const n = b / k; const q = e / f; const p = Math.min(n, q); const m = Math.max(n, q); d == 'meet' && (k *=
p, f *= p); d == 'slice' && (k *= m, f *= m); h = new a.Property('refX', h); l = new a.Property('refY', l); h.hasValue() && l.hasValue() ? c.translate(-p * h.Length.toPixels('x'), -p * l.Length.toPixels('y')) : (o.match(/^xMid/) && (d == 'meet' && p == q || d == 'slice' && m == q) && c.translate(b / 2 - k / 2, 0), o.match(/YMid$/) && (d == 'meet' && p == n || d == 'slice' && m == n) && c.translate(0, e / 2 - f / 2), o.match(/^xMax/) && (d == 'meet' && p == q || d == 'slice' && m == q) && c.translate(b - k, 0), o.match(/YMax$/) && (d == 'meet' && p == n || d == 'slice' && m == n) && c.translate(0, e - f)); o == 'none' ? c.scale(n,
            ...
            this.attributes = {}; this.styles = {}; this.children = []; this.attribute = function(b, d) { let c = this.attributes[b]; if (c != null) return c; c = new a.Property(b, ''); d == !0 && (this.attributes[b] = c); return c }; this.style = function(b, d) {
...
function() {}; this.clearContext = function() {}; this.renderChildren = function(b) { for (let a = 0; a < this.children.length; a++) this.children[a].render(b) }; this.addChild = function(b, d) { let c = b; d && (c = a.CreateElement(b)); c.parent = this; this.children.push(c) }; if (c != null && c.nodeType == 1) {
...
this.style('stroke-linecap').value
            ...
            this.base = a.Element.RenderedElementBase; this.base(c); this.path = function(d) {
                ...
                this.path(d); a.Mouse.checkPath(this, d); d.fillStyle != '' && d.fill(); d.strokeStyle != '' && d.stroke(); const b = this.getMarkers(); if (b != null) {
...
(c = this.style('marker-end').Definition.getDefinition(), c.render(d, b[b.length - 1][0], b[b.length - 1][1]))
            ...
            this.setContext = function(d) {
                d.strokeStyle = 'rgba(0,0,0,0)'; d.lineCap = 'butt'; d.lineJoin = 'miter'; d.miterLimit = 4; this.baseSetContext(d); this.attribute('x').hasValue() && this.attribute('y').hasValue() && d.translate(this.attribute('x').Length.toPixels('x'), this.attribute('y').Length.toPixels('y')); var b = a.ViewPort.width(); var c = a.ViewPort.height(); if (typeof this.root === 'undefined' && this.attribute('width').hasValue() && this.attribute('height').hasValue()) {
                    ...
                    var e = a.ToNumberArray(this.attribute('viewBox').value); var f = e[0]; const g = e[1]; var b = e[2]; var c = e[3]; a.AspectRatio(d, this.attribute('preserveAspectRatio').value, a.ViewPort.width(), b, a.ViewPort.height(),
...
new a.Element.PathElementBase(); a.Element.line = function(c) {
            ...
            this.base = a.Element.PathElementBase; this.base(c); this.points = a.CreatePath(this.attribute('points').value); this.path = function(d) {
                const b = new a.BoundingBox(this.points[0].x, this.points[0].y); d != null && (d.beginPath(), d.moveTo(this.points[0].x, this.points[0].y)); for (let c = 1; c < this.points.length; c++) {
            ...
            this.base = a.Element.polyline; this.base(c); this.basePath = this.path; this.path = function(a) {
                const b = this.basePath(a); a != null && (a.lineTo(this.points[0].x,
...
1] == null && (this.angles[this.angles.length - 1] = this.points[this.points.length - 1].angleTo(d)); this.addMarkerAngle(b, a == null ? null : a.angleTo(b))
                }; this.addMarkerAngle = function(b, a) { this.points.push(b); this.angles.push(a) }; this.getMarkerPoints = function() { return this.points }; this.getMarkerAngles = function() { for (let b = 0; b < this.angles.length; b++) if (this.angles[b] == null) for (let a = b + 1; a < this.angles.length; a++) if (this.angles[a] != null) { this.angles[b] = this.angles[a]; break } return this.angles }
            }(c); this.path = function(d) {
                const b =
this.PathParser; b.reset(); const c = new a.BoundingBox(); for (d != null && d.beginPath(); !b.isEnd();) {
...
b.current; var f = b.getPoint(); var j = b.getAsControlPoint(); var e = b.getAsCurrentPoint(); b.addMarker(e, j, f); c.addBezierCurve(g.x, g.y, f.x, f.y, j.x, j.y, e.x, e.y); d != null && d.bezierCurveTo(f.x, f.y, j.x, j.y, e.x, e.y)
                            ...
                            var g = b.current; let h = b.getScalar(); let l = b.getScalar(); var f = b.getScalar() * (Math.PI / 180); var o = b.getScalar(); var j = b.getScalar()
...
p.x - Math.sin(f) * p.y, (g.y + e.y) / 2 + Math.sin(f) * p.x + Math.cos(f) * p.y); var m = function(b, a) { return (b[0] * a[0] + b[1] * a[1]) / (Math.sqrt(Math.pow(b[0], 2) + Math.pow(b[1], 2)) * Math.sqrt(Math.pow(a[0], 2) + Math.pow(a[1], 2))) }; const s = function(b, a) { return (b[0] * a[1] < b[1] * a[0] ? -1 : 1) * Math.acos(m(b, a)) }; var o = s([1, 0], [(n.x - p.x) / h, (n.y - p.y) / l]); var q = [(n.x - p.x) / h, (n.y - p.y) / l]; var p = [(-n.x - p.x) / h, (-n.y - p.y) / l]; var n = s(q, p); if (m(q, p) <= -1)n = Math.PI; m(q, p) >= 1 && (n = 0); j == 0 && n > 0 && (n -= 2 * Math.PI); j == 1 && n < 0 && (n += 2 * Math.PI); q = new a.Point(g.x - h * Math.cos((o + n) /
2), g.y - l * Math.sin((o + n) / 2)); b.addMarkerAngle(q, (o + n) / 2 + (j == 0 ? 1 : -1) * Math.PI / 2); b.addMarkerAngle(e, n + (j == 0 ? 1 : -1) * Math.PI / 2); c.addPoint(e.x, e.y); d != null && (m = h > l ? h : l, e = h > l ? 1 : h / l, h = h > l ? l / h : 1, d.translate(g.x, g.y), d.rotate(f), d.scale(e, h), d.arc(0, 0, m, o, o + n, 1 - j), d.scale(1 / e, 1 / h), d.rotate(-f), d.translate(-g.x, -g.y))
            ...
            this.base = a.Element.ElementBase; this.base(c); this.createPattern = function(d) {
                const b = new a.Element.svg(); b.attributes.viewBox = new a.Property('viewBox', this.attribute('viewBox').value); b.attributes.x = new a.Property('x', this.attribute('x').value); b.attributes.y = new a.Property('y', this.attribute('y').value); b.attributes.width = new a.Property('width', this.attribute('width').value); b.attributes.height = new a.Property('height',
                    this.attribute('height').value); b.children = this.children; const c = document.createElement('canvas'); c.width = this.attribute('width').Length.toPixels('x'); c.height = this.attribute('height').Length.toPixels('y'); b.render(c.getContext('2d')); return d.createPattern(c, 'repeat')
            }
...
'auto' && d.rotate(c); this.attribute('markerUnits').valueOrDefault('strokeWidth') == 'strokeWidth' && d.scale(d.lineWidth, d.lineWidth); d.save(); const e = new a.Element.svg(); e.attributes.viewBox = new a.Property('viewBox', this.attribute('viewBox').value); e.attributes.refX = new a.Property('refX', this.attribute('refX').value); e.attributes.refY = new a.Property('refY', this.attribute('refY').value); e.attributes.width = new a.Property('width', this.attribute('markerWidth').value); e.attributes.height = new a.Property('height',
...
(c = this.attribute('xlink:href').Definition.getDefinition()); for (var e = this.getGradient(d, b), f = 0; f < c.stops.length; f++)e.addColorStop(c.stops[f].offset, c.stops[f].color); if (this.attribute('gradientTransform').hasValue()) {
        ...
        a.Element.GradientBase.prototype = new a.Element.ElementBase(); a.Element.linearGradient = function(c) {
            this.base = a.Element.GradientBase; this.base(c); this.getGradient = function(a, b) {
                var c = b.getBoundingBox(); const e = this.gradientUnits == 'objectBoundingBox' ? c.x() + c.width() * this.attribute('x1').numValue() : this.attribute('x1').Length.toPixels('x'); const f = this.gradientUnits == 'objectBoundingBox' ? c.y() + c.height() * this.attribute('y1').numValue() : this.attribute('y1').Length.toPixels('y'); const g = this.gradientUnits == 'objectBoundingBox'
            ...
            this.base = a.Element.GradientBase; this.base(c); this.getGradient = function(a, b) {
                let c = b.getBoundingBox(); const e = this.gradientUnits == 'objectBoundingBox'
                    ? c.x() + c.width() * this.attribute('cx').numValue() : this.attribute('cx').Length.toPixels('x'); const f = this.gradientUnits == 'objectBoundingBox' ? c.y() + c.height() * this.attribute('cy').numValue() : this.attribute('cy').Length.toPixels('y'); let g = e; let j = f; this.attribute('fx').hasValue() && (g = this.gradientUnits == 'objectBoundingBox' ? c.x() + c.width() * this.attribute('fx').numValue() : this.attribute('fx').Length.toPixels('x')); this.attribute('fy').hasValue() && (j = this.gradientUnits == 'objectBoundingBox' ? c.y() + c.height() * this.attribute('fy').numValue()
...
a.Element.animate; this.base(c)
            ...
            this.base = a.Element.ElementBase; this.base(c); this.horizAdvX = this.attribute('horiz-adv-x').numValue(); this.isArabic = this.isRTL = !1; this.missingGlyph = this.fontFace = null; this.glyphs = []; for (c = 0; c < this.children.length; c++) {
                const d = this.children[c]; if (d.type == 'font-face') this.fontFace = d, d.style('font-family').hasValue() && (a.Definitions[d.style('font-family').value] = this); else if (d.type == 'missing-glyph') {
                    this.missingGlyph =
d
...
function(b) {
    for (let a = this.style('text-anchor').valueOrDefault('start'), c = this.attribute('x').Length.toPixels('x'), d = this.attribute('y').Length.toPixels('y'), j = 0; j < this.children.length; j++) {
        const h = this.children[j]; h.attribute('x').hasValue() ? h.x = h.attribute('x').Length.toPixels('x') : (h.attribute('dx').hasValue() && (c += h.attribute('dx').Length.toPixels('x')), h.x = c); c = h.measureText(b); if (a != 'start' && (j == 0 || h.attribute('x').hasValue())) {
            for (var l = c, o = j + 1; o < this.children.length; o++) {
                const n = this.children[o]
            ...
            this.base = a.Element.RenderedElementBase; this.base(c); this.getGlyph = function(a, b, c) {
                const e = b[c]; let f = null; if (a.isArabic) {
...
1] == ' ') && c < b.length - 2 && b[c + 1] != ' ')g = 'terminal'; c > 0 && b[c - 1] != ' ' && c < b.length - 2 && b[c + 1] != ' ' && (g = 'medial'); if (c > 0 && b[c - 1] != ' ' && (c == b.length - 1 || b[c + 1] == ' '))g = 'initial'; typeof a.glyphs[e] !== 'undefined' && (f = a.glyphs[e][g], f == null && a.glyphs[e].type == 'glyph' && (f = a.glyphs[e]))
                ...
                const b = this.parent.style('font-family').Definition.getDefinition(); if (b != null) {
                    const k = this.parent.style('font-size').numValueOrDefault(a.Font.Parse(a.ctx.font).fontSize)
                    const e = this.parent.style('font-style').valueOrDefault(a.Font.Parse(a.ctx.font).fontStyle); let f = this.getText(); b.isRTL && (f = f.split('').reverse()
                        .join('')); for (let g = a.ToNumberArray(this.parent.attribute('dx').value), j = 0; j < f.length; j++) {
                        const h = this.getGlyph(b, f, j); const l = k / b.fontFace.unitsPerEm; c.translate(this.x, this.y); c.scale(l, -l); const o = c.lineWidth; c.lineWidth = c.lineWidth * b.fontFace.unitsPerEm / k; e == 'italic' && c.transform(1, 0, 0.4, 1, 0, 0); h.render(c); e == 'italic' && c.transform(1, 0, -0.4, 1, 0, 0); c.lineWidth = o; c.scale(1 /
l, -1 / l); c.translate(-this.x, -this.y); this.x += k * (h.horizAdvX || b.horizAdvX) / b.fontFace.unitsPerEm; typeof g[j] !== 'undefined' && !isNaN(g[j]) && (this.x += g[j])
                ...
                let b = this.parent.style('font-family').Definition.getDefinition(); if (b != null) {
                    ...
                    let k = 0; let e = this.getText(); b.isRTL && (e = e.split('').reverse()
                        .join('')); for (let f = a.ToNumberArray(this.parent.attribute('dx').value), g = 0; g < e.length; g++) { const j = this.getGlyph(b, e, g); k += (j.horizAdvX || b.horizAdvX) * c / b.fontFace.unitsPerEm; typeof f[g] !== 'undefined' && !isNaN(f[g]) && (k += f[g]) } return k
        ...
        a.Element.tspan = function(c) { this.base = a.Element.TextElementBase; this.base(c); this.text = c.nodeType == 3 ? c.nodeValue : c.childNodes.length > 0 ? c.childNodes[0].nodeValue : c.text; this.getText = function() { return this.text } }; a.Element.tspan.prototype = new a.Element.TextElementBase(); a.Element.tref = function(c) { this.base = a.Element.TextElementBase; this.base(c); this.getText = function() { const a = this.attribute('xlink:href').Definition.getDefinition(); if (a != null) return a.children[0].getText() } }; a.Element.tref.prototype = new a.Element.TextElementBase()
        a.Element.a = function(c) {
            this.base = a.Element.TextElementBase; this.base(c); this.hasText = !0; for (let d = 0; d < c.childNodes.length; d++) if (c.childNodes[d].nodeType != 3) this.hasText = !1; this.text = this.hasText ? c.childNodes[0].nodeValue : ''; this.getText = function() { return this.text }; this.baseRenderChildren = this.renderChildren; this.renderChildren = function(b) {
                if (this.hasText) {
            ...
            this.base = a.Element.RenderedElementBase; this.base(c); a.Images.push(this); this.img = document.createElement('img'); this.loaded = !1; const d = this; this.img.onload =
function() { d.loaded = !0 }; this.img.src = this.attribute('xlink:href').value; this.renderChildren = function(b) { const c = this.attribute('x').Length.toPixels('x'); const d = this.attribute('y').Length.toPixels('y'); const f = this.attribute('width').Length.toPixels('x'); const g = this.attribute('height').Length.toPixels('y'); f == 0 || g == 0 || (b.save(), b.translate(c, d), a.AspectRatio(b, this.attribute('preserveAspectRatio').value, f, this.img.width, g, this.img.height, 0, 0), b.drawImage(this.img, 0, 0), b.restore()) }
        ...
        a.Element.g = function(c) { this.base = a.Element.RenderedElementBase; this.base(c); this.getBoundingBox = function() { for (var c = new a.BoundingBox(), b = 0; b < this.children.length; b++)c.addBoundingBox(this.children[b].getBoundingBox()); return c } }; a.Element.g.prototype = new a.Element.RenderedElementBase(); a.Element.symbol = function(c) {
            this.base = a.Element.RenderedElementBase; this.base(c); this.baseSetContext = this.setContext; this.setContext = function(c) {
                this.baseSetContext(c); if (this.attribute('viewBox').hasValue()) {
                    const b =
a.ToNumberArray(this.attribute('viewBox').value); const k = b[0]; const e = b[1]; width = b[2]; height = b[3]; a.AspectRatio(c, this.attribute('preserveAspectRatio').value, this.attribute('width').Length.toPixels('x'), width, this.attribute('height').Length.toPixels('y'), height, k, e); a.ViewPort.SetCurrent(b[2], b[3])
                }
            }
                        ...
                        let f = a.trim(k[e]); if (f != '') {
                            for (var g = {}, j = 0; j < b.length; j++) { var h = b[j].indexOf(':'); var l = b[j].substr(0, h); var h = b[j].substr(h + 1, b[j].length - h); l != null && h != null && (g[a.trim(l)] = new a.Property(a.trim(l), a.trim(h))) }a.Styles[f] = g; if (f == '@font-face') {
                                f = g['font-family'].value.replace(/"/g,
                                    ''); g = g.src.value.split(','); for (j = 0; j < g.length; j++) if (g[j].indexOf('format("svg")') > 0) { l = g[j].indexOf('url'); h = g[j].indexOf(')', l); l = g[j].substr(l + 5, h - l - 6); l = a.parseXml(a.ajax(l)).getElementsByTagName('font'); for (h = 0; h < l.length; h++) { const o = a.CreateElement(l[h]); a.Definitions[f] = o } }
                            }
                        }
                    }
                }
            ...
            }; this.getDefinition = function() { const a = this.attribute('xlink:href').Definition.getDefinition(); if (this.attribute('width').hasValue())a.attribute('width', !0).value = this.attribute('width').value; if (this.attribute('height').hasValue())a.attribute('height', !0).value = this.attribute('height').value; return a }; this.path = function(a) {
                const b =
this.getDefinition(); b != null && b.path(a)
            }; this.renderChildren = function(a) { const b = this.getDefinition(); b != null && b.render(a) }
            ...
            this.base = a.Element.ElementBase; this.base(c); this.apply = function(a, b) {
                const c = this.attribute('x').Length.toPixels('x'); const e = this.attribute('y').Length.toPixels('y'); const f = this.attribute('width').Length.toPixels('x'); const g = this.attribute('height').Length.toPixels('y'); const j = b.attribute('mask').value; b.attribute('mask').value =
''; const h = document.createElement('canvas'); h.width = c + f; h.height = e + g; const l = h.getContext('2d'); this.renderChildren(l); const o = document.createElement('canvas'); o.width = c + f; o.height = e + g; const n = o.getContext('2d'); b.render(n); n.globalCompositeOperation = 'destination-in'; n.fillStyle = l.createPattern(h, 'no-repeat'); n.fillRect(0, 0, c + f, e + g); a.fillStyle = n.createPattern(o, 'no-repeat'); a.fillRect(0, 0, c + f, e + g); b.attribute('mask').value = j
            }; this.render = function() {}
        }; a.Element.mask.prototype = new a.Element.ElementBase(); a.Element.clipPath =
function(c) { this.base = a.Element.ElementBase; this.base(c); this.apply = function(a) { for (let b = 0; b < this.children.length; b++) this.children[b].path && (this.children[b].path(a), a.clip()) }; this.render = function() {} }; a.Element.clipPath.prototype = new a.Element.ElementBase(); a.Element.filter = function(c) {
            this.base = a.Element.ElementBase; this.base(c); this.apply = function(a, b) {
                let c = b.getBoundingBox(); let e = this.attribute('x').Length.toPixels('x'); let f = this.attribute('y').Length.toPixels('y'); if (e == 0 || f == 0)e = c.x1, f = c.y1; let g =
this.attribute('width').Length.toPixels('x'); let j = this.attribute('height').Length.toPixels('y'); if (g == 0 || j == 0)g = c.width(), j = c.height(); c = b.style('filter').value; b.style('filter').value = ''; const h = 0.2 * g; const l = 0.2 * j; const o = document.createElement('canvas'); o.width = g + 2 * h; o.height = j + 2 * l; const n = o.getContext('2d'); n.translate(-e + h, -f + l); b.render(n); for (let q = 0; q < this.children.length; q++) this.children[q].apply(n, 0, 0, g + 2 * h, j + 2 * l); a.drawImage(o, 0, 0, g + 2 * h, j + 2 * l, e - h, f - l, g + 2 * h, j + 2 * l); b.style('filter', !0).value = c
            }; this.render =
function() {}
            ...
            function d(a, c, d, f, g) {
                for (let j = 0; j < g; j++) {
                    for (let h = 0; h < f; h++) {
                        for (let l = a[j * f * 4 + h * 4 + 3] / 255, o = 0; o < 4; o++) {
                            for (var n = d[0] * (l == 0 ? 255 : a[j * f * 4 + h * 4 + o]) * (l == 0 || o == 3 ? 1 : l), q = 1; q < d.length; q++) {
                                var p = Math.max(h - q, 0); let m = a[j * f * 4 + p * 4 + 3] / 255; var p = Math.min(h + q, f - 1); var p = a[j * f * 4 + p * 4 + 3] / 255; const s = d[q]; var r; m == 0 ? r = 255 : (r = Math.max(h - q, 0), r = a[j * f * 4 + r * 4 + o]); m = r * (m == 0 || o == 3 ? 1 : m); p == 0 ? r = 255 : (r = Math.min(h + q, f - 1), r = a[j * f * 4 + r * 4 + o]); n +=
s * (m + r * (p == 0 || o == 3 ? 1 : p))
                            }c[h * g * 4 + j * 4 + o] = n
                        }
                    }
                }
            } this.base = a.Element.ElementBase; this.base(c); this.apply = function(a, c, e, f, g) { var e = this.attribute('stdDeviation').numValue(); var c = a.getImageData(0, 0, f, g); var e = Math.max(e, 0.01); let j = Math.ceil(e * 4) + 1; mask = []; for (var h = 0; h < j; h++)mask[h] = Math.exp(-0.5 * (h / e) * (h / e)); e = mask; j = 0; for (h = 1; h < e.length; h++)j += Math.abs(e[h]); j = 2 * j + Math.abs(e[0]); for (h = 0; h < e.length; h++)e[h] /= j; tmp = []; d(c.data, tmp, e, f, g); d(tmp, c.data, e, g, f); a.clearRect(0, 0, f, g); a.putImageData(c, 0, 0) }
...
new a.Element.feGaussianBlur(); a.Element.title = function() {}; a.Element.title.prototype = new a.Element.ElementBase(); a.Element.desc = function() {}; a.Element.desc.prototype = new a.Element.ElementBase(); a.Element.MISSING = function(a) { console.log("ERROR: Element '" + a.nodeName + "' not yet implemented.") }; a.Element.MISSING.prototype = new a.Element.ElementBase(); a.CreateElement = function(c) {
            ...
            a.init(c); const b = function(a) { for (let b = c.canvas; b;)a.x -= b.offsetLeft, a.y -= b.offsetTop, b = b.offsetParent; window.scrollX && (a.x += window.scrollX); window.scrollY && (a.y += window.scrollY); return a }; if (a.opts.ignoreMouse != !0) {
            ...
            } const k = a.CreateElement(d.documentElement); let e = k.root = !0; const f = function() {
...
k.style('height').Length.toPixels('y'), c.canvas.style.height = c.canvas.height + 'px'
                ...
                } const b = c.canvas.clientWidth || c.canvas.width; const d = c.canvas.clientHeight || c.canvas.height; a.ViewPort.SetCurrent(b, d); if (a.opts != null && a.opts.offsetX != null)k.attribute('x', !0).value = a.opts.offsetX; if (a.opts != null && a.opts.offsetY != null)k.attribute('y', !0).value = a.opts.offsetY; if (a.opts != null && a.opts.scaleWidth != null && a.opts.scaleHeight != null) {
                    let f = 1; let g = 1; k.attribute('width').hasValue() && (f = k.attribute('width').Length.toPixels('x') /
a.opts.scaleWidth); k.attribute('height').hasValue() && (g = k.attribute('height').Length.toPixels('y') / a.opts.scaleHeight); k.attribute('width', !0).value = a.opts.scaleWidth; k.attribute('height', !0).value = a.opts.scaleHeight; k.attribute('viewBox', !0).value = '0 0 ' + b * f + ' ' + d * g; k.attribute('preserveAspectRatio', !0).value = 'none'
            ...
            }; let g = !0; a.ImagesLoaded() && (g = !1, f())
            a.intervalID = setInterval(function() { let b = !1; g && a.ImagesLoaded() && (g = !1, b = !0); a.opts.ignoreMouse != !0 && (b |= a.Mouse.hasEvents()); if (a.opts.ignoreAnimation != !0) for (let c = 0; c < a.Animations.length; c++)b |= a.Animations[c].update(1E3 / a.FRAMERATE); a.opts != null && typeof a.opts.forceRedraw === 'function' && a.opts.forceRedraw() == !0 && (b = !0); b && (f(), a.Mouse.runEvents()) }, 1E3 / a.FRAMERATE)
        }; a.stop = function() { a.intervalID && clearInterval(a.intervalID) }; a.Mouse = new function() {
            this.events = []; this.hasEvents = function() {
                return this.events.length !=
0
            }; this.onclick = function(a, d) { this.events.push({ type: 'onclick', x: a, y: d, run: function(a) { if (a.onclick)a.onclick() } }) }; this.onmousemove = function(a, d) { this.events.push({ type: 'onmousemove', x: a, y: d, run: function(a) { if (a.onmousemove)a.onmousemove() } }) }; this.eventElements = []; this.checkPath = function(a, d) { for (let b = 0; b < this.events.length; b++) { const k = this.events[b]; d.isPointInPath && d.isPointInPath(k.x, k.y) && (this.eventElements[b] = a) } }; this.checkBoundingBox = function(a, d) {
                for (let b = 0; b < this.events.length; b++) {
                    const k =
this.events[b]; d.isPointInBox(k.x, k.y) && (this.eventElements[b] = a)
                }
            }; this.runEvents = function() { a.ctx.canvas.style.cursor = ''; for (let c = 0; c < this.events.length; c++) for (let d = this.events[c], b = this.eventElements[c]; b;)d.run(b), b = b.parent; this.events = []; this.eventElements = [] }
        }(); return a
        ...
        if (a == null && c == null && d == null) {
            for (var c = document.getElementsByTagName('svg'), b = 0; b < c.length; b++) {
                a = c[b]; d = document.createElement('canvas'); d.width = a.clientWidth; d.height = a.clientHeight; a.parentNode.insertBefore(d,
                    a); a.parentNode.removeChild(a); const k = document.createElement('div'); k.appendChild(a); canvg(d, k.innerHTML)
            }
...
(function(m) {
    const a = m.css; const c = m.CanVGRenderer; const d = m.SVGRenderer; const b = m.extend; const k = m.merge; const e = m.addEvent; const f = m.createElement; const g = m.discardElement; b(c.prototype, d.prototype); b(c.prototype, { create: function(a, b, c, d) { this.setContainer(b, c, d); this.configure(a) }, setContainer: function(a, b, c) {
        var d = a.style; const e = a.parentNode; const g = d.left; var d = d.top; const k = a.offsetWidth; const m = a.offsetHeight; const s = { visibility: 'hidden', position: 'absolute' }; this.init.apply(this, [a, b, c]); this.canvas = f('canvas', { width: k, height: m }, { position: 'relative', left: g, top: d }, a)

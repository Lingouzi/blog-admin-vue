    ...
    function build() {
        ...
        svg.Property = function(name, value) {
            ...
            this.Color = {
                ...
                addOpacity: function(opacity) {
                    ...
                    if (opacity != null && opacity != '') {
                        const color = new RGBColor(that.value)
                        if (color.ok) {
            ...
            this.Definition = {
                ...
                getDefinition: function() {
                    const name = that.value.replace(/^(url\()?#([^\)]+)\)?$/, '$2')
        ...
        svg.Transform = function(v) {
            ...
            this.Type.rotate = function(s) {
                ...
                this.applyToPoint = function(p) {
                    const a = this.angle.Angle.toRadians()
                    p.applyTransform([1, 0, 0, 1, this.p.x || 0.0, this.p.y || 0.0])
            ...
            const data = svg.trim(svg.compressSpaces(v)).split(/\s(?=[a-z])/)
            for (let i = 0; i < data.length; i++) {
                const type = data[i].split('(')[0]
        ...
        svg.Element.ElementBase = function(node) {
            ...
            this.render = function(ctx) {
                ...
                if (this.attribute('mask').hasValue()) {
                    const mask = this.attribute('mask').Definition.getDefinition()
                    if (mask != null) mask.apply(ctx, this)
                } else if (this.style('filter').hasValue()) {
                    const filter = this.style('filter').Definition.getDefinition()
        ...
        svg.Element.RenderedElementBase = function(node) {
            ...
            this.setContext = function(ctx) {
                ...
                if (this.attribute('clip-path').hasValue()) {
                    const clip = this.attribute('clip-path').Definition.getDefinition()
        ...
        svg.Element.rect = function(node) {
            ...
            this.path = function(ctx) {
                const x = this.attribute('x').Length.toPixels('x')
                const y = this.attribute('y').Length.toPixels('y')
                const width = this.attribute('width').Length.toPixels('x')
                const height = this.attribute('height').Length.toPixels('y')
        ...
        svg.Element.circle = function(node) {
            ...
            this.path = function(ctx) {
                const cx = this.attribute('cx').Length.toPixels('x')
                const cy = this.attribute('cy').Length.toPixels('y')
                const r = this.attribute('r').Length.toPixels()
        ...
        svg.Element.ellipse = function(node) {
            ...
            this.path = function(ctx) {
                ...
                const rx = this.attribute('rx').Length.toPixels('x')
                const ry = this.attribute('ry').Length.toPixels('y')
                const cx = this.attribute('cx').Length.toPixels('x')
                const cy = this.attribute('cy').Length.toPixels('y')
        ...
        svg.Element.animate = function(node) {
            ...
            this.calcValue = function() {
                const from = this.attribute('from').numValue()
                const to = this.attribute('to').numValue()
        ...
        svg.Element.animateColor = function(node) {
            ...
            this.calcValue = function() {
                const from = new RGBColor(this.attribute('from').value)
                const to = new RGBColor(this.attribute('to').value)

                if (from.ok && to.ok) {
                    // tween color linearly
                    const r = from.r + (to.r - from.r) * this.progress()
                    const g = from.g + (to.g - from.g) * this.progress()
                    const b = from.b + (to.b - from.b) * this.progress()
        ...
        svg.Element.text = function(node) {
            ...
            this.renderChildren = function(ctx) {
                ...
                for (let i = 0; i < this.children.length; i++) {
                    ...
                    const childLength = child.measureText(ctx)
                    if (textAnchor != 'start' && (i == 0 || child.attribute('x').hasValue())) { // new group?
                        // loop through rest of children
                        let groupLength = childLength
                        for (let j = i + 1; j < this.children.length; j++) {
                            const childInGroup = this.children[j]
        ...
        svg.Element.TextElementBase = function(node) {
            ...
            this.renderChildren = function(ctx) {
                const customFont = this.parent.style('font-family').Definition.getDefinition()
                if (customFont != null) {
        ...
        svg.Element.tref = function(node) {
            ...
            this.getText = function() {
                const element = this.attribute('xlink:href').Definition.getDefinition()
        ...
        svg.Element.use = function(node) {
            ...
            this.path = function(ctx) {
                const element = this.getDefinition()
        ...
        svg.Element.filter = function(node) {
            ...
            this.apply = function(ctx, element) {
                ...
                const px = extraPercent * width
                const py = extraPercent * height
        ...
        svg.Element.feGaussianBlur = function(node) {
            ...
            function make_fgauss(sigma) {
                ...
                const len = Math.ceil(sigma * 4.0) + 1
                mask = []
                for (let i = 0; i < len; i++) {
            ...
            function convolve_even(src, dst, mask, width, height) {
			  for (let y = 0; y < height; y++) {
				  ...
				  const a = imGet(src, x, y, width, height, 3) / 255
				  for (let rgba = 0; rgba < 4; rgba++) {
					  let sum = mask[0] * (a == 0 ? 255 : imGet(src, x, y, width, height, rgba)) * (a == 0 || rgba == 3 ? 1 : a)
					  for (let i = 1; i < mask.length; i++) {
                                const a1 = imGet(src, Math.max(x - i, 0), y, width, height, 3) / 255
					    const a2 = imGet(src, Math.min(x + i, width - 1), y, width, height, 3) / 255
                                sum += mask[i] *
        ...
        svg.loadXmlDoc = function(ctx, dom) {
            ...
            const draw = function() {
                ...
                const cWidth = ctx.canvas.clientWidth || ctx.canvas.width
                const cHeight = ctx.canvas.clientHeight || ctx.canvas.height
...
(function (Highcharts) { // encapsulate
    ...
    const PX = 'px'
    const css = Highcharts.css
    const CanVGRenderer = Highcharts.CanVGRenderer
    const SVGRenderer = Highcharts.SVGRenderer
    const extend = Highcharts.extend
    const merge = Highcharts.merge
    const addEvent = Highcharts.addEvent
    const createElement = Highcharts.createElement
    const discardElement = Highcharts.discardElement
    ...
    extend(CanVGRenderer.prototype, {
        ...
        setContainer: function (container, chartWidth, chartHeight) {
            ...
            const hiddenSvg = createElement(DIV, {
                width: containerOffsetWidth,
                height: containerOffsetHeight
            }, {
                visibility: HIDDEN,
        ...
        configure: function (chart) {
            const renderer = this
            const options = chart.options.tooltip
            const borderWidth = options.borderWidth
            const tooltipDiv = renderer.ttDiv
            let tooltipDivStyle = options.style
            const tooltipLine = renderer.ttLine
            const padding = parseInt(tooltipDivStyle.padding, 10)
            ...
            addEvent(chart, 'tooltipRefresh', function (args) {
                ...
                const offsetLeft = chartContainer.offsetLeft
                const offsetTop = chartContainer.offsetTop
                let position
